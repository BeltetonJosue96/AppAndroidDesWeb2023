...
class Registrar : AppCompatActivity() {
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (requestCode == CODIGO_GALERIA && resultCode == Activity.RESULT_OK) {
            ...
            imagenBitmap.compress(Bitmap.CompressFormat.JPEG, 70, byteArrayOutputStream)
    ...
    private fun enviarDatos() {
        ...
        if (token == null) {
            runOnUiThread {
                Toast.makeText(this@Registrar, "No se ha iniciado sesión", Toast.LENGTH_SHORT).show()
            }
            return
        }
        ...
        OkHttpClient().newCall(request).enqueue(object : Callback {
            override fun onFailure(call: Call, e: IOException) {
                runOnUiThread {
                    Toast.makeText(this@Registrar, "Lo sentimos existen fallos de comunicación, verifique su conexión a Internet", Toast.LENGTH_SHORT).show()
                }
            }
            ...
            override fun onResponse(call: Call, response: Response) {
                if (response.isSuccessful) {
                    ...
                    runOnUiThread {
                        Toast.makeText(this@Registrar, mensaje, Toast.LENGTH_SHORT).show()
                    }
                } else {
                    runOnUiThread {
                        Toast.makeText(this@Registrar, "Lo sentimos, no se pudo completar la petición", Toast.LENGTH_SHORT).show()
                    }
                }
            }
        })
